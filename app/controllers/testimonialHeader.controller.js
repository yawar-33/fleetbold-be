...
const Service = db.testimonial;

// Get complete membership section (header + benefits)
exports.getCompleteTestimonialSection = async (_req, res) => {
  try {
    let header = await TestimonialHeader.findOne({ isActive: true }).populate('testimonialList');
    ...
    if (!header) {
      ...
      await header.save();
      await header.populate('testimonialList');
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        testimonialList: header.testimonialList
      }
    });
  } catch (err) {
